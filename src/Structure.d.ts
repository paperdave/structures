...
export declare class Structure<Props extends Dict<StructureProperty> = {}, Methods = {}> {
  ...
  prop<Key extends string, Type>(
    ...
    Props & {
      [key in Key]: StructureProperty<
        Type,
        Extract<Type, null | undefined> extends never ? true : false
      >;
    },
    Methods
  >;
  prop<Key extends string, Type, Options extends PropertyOptions<Type>>(
    key: Key,
    type: DataType<Type>,
    options: Options
  ): Structure<
    Props & {
      [key in Key]: StructureProperty<
        Type,
        Options['default'] extends undefined
  ...
  method<
    Key extends string,
    Func extends (this: StructureInstance<Props, Methods>, ...args: any[]) => any
  >(key: Key, func: Func): Structure<Props, Methods & ObjWithMethod<Key, Func>>;
  mixin<T2 extends Dict<StructureProperty>, M2>(
    mixin: Structure<T2, M2> | StructureType<T2, M2>
  ): Structure<Props & T2, Methods & M2>;
  create(opts?: CreateOptions<Props, Methods>): StructureType<Props, Methods>;
}
declare type StructureType<T extends Dict<StructureProperty>, M> = DataType<
  StructureInstance<T, M>
> & {
...
export declare type RequiredKeyNames<T, A> = {
  [K in keyof T]: T[K] extends StructureProperty<infer T, infer R>
    ? R extends A
      ? K
      : never
    : never;
